 ...
 */
//ex. of extension functions
//fun String.isLongerThan(i: Int): Boolean{
//    return this.length > i
//}

// model is what will change what is being shown
// singleton = only one object allowed to be made, cannot have companion objects
object GameModel {
    ...
    fun resetGame() {
        wastePile.clear()
        foundationPiles.forEach { it.reset() }// or it.reset()
        deck.reset()
        //have access to the value at the index and the item itself
    ...
    fun debugPrint() {
        var firstLine = if (wastePile.size > 0) "${wastePile.last()}" else "__"
        firstLine = firstLine.padEnd(18)
        foundationPiles.forEach {
            firstLine += if (it.cards.isNotEmpty()) "${it.cards.last()}" else "__"
            firstLine += "   "
